...
public class TC_2_Assi_18 {
    ...
    public void Assignment_18() {
        ...
        menucardpage.quantityForGivenDish(dishName, 2);
        ...
        System.out.println("Search orderId from myOrderPageFromSearch bar");
        MyOrdersPage myorderspage = new MyOrdersPage();
        myorderspage.waitForPageLoad();
        String orderIDFromMyOrdersPage = myorderspage.getOrderIdFromSearchBar(orderIDFromOrderSuccessPage);
        softassert.assertEquals(orderIDFromOrderSuccessPage, orderIDFromMyOrdersPage, "Order id is not matching");

        System.out.println("VERIFY - verify Order details");
        orderIDFromMyOrdersPage = myorderspage.getOrderIdFromSearchBar(orderIDFromOrderSuccessPage);
        Map<String, String> orderDetailsMap = myorderspage.getOrderDetails(orderIDFromMyOrdersPage);
        System.out.println("order Details Map :-"+orderDetailsMap);
        softassert.assertEquals(orderDetailsMap.get("Order #"), orderIDFromMyOrdersPage);
        softassert.assertEquals(orderDetailsMap.get("Date"), expectedDateAndTime);
        softassert.assertTrue((orderDetailsMap.get("Restaurant").contains(restraurantName)));
        softassert.assertEquals(orderDetailsMap.get("Total"), totalPayableAmt);
        softassert.assertTrue(getOrderStatusList().contains(orderDetailsMap.get("Status")),"Displayed Status was not in the list"); //or
